...
namespace Tavenem.Chemistry
{
    ...
    public struct Formula : ISerializable, IEquatable<Formula>
    {
        /// <summary>
        ...
        private Formula(SerializationInfo info, StreamingContext context) : this(
            (short?)info.GetValue(nameof(Charge), typeof(short)) ?? default,
            (Dictionary<string, ushort>?)info.GetValue(nameof(_isotopes), typeof(Dictionary<string, ushort>)) ?? new Dictionary<string, ushort>())
        { }
        ...
        /// cref="short.MaxValue"/> or <see cref="short.MinValue"/>, or the amount of any nuclide
        /// exceeds <see cref="ushort.MaxValue"/>.</exception>
        public static Formula Divide(Formula value, HugeNumber divisor)
        {
            if (divisor <= 0)
            {
                throw new ArgumentOutOfRangeException(nameof(divisor));
            ...
            foreach (var isotope in combined.ToList())
            {
                var amount = HugeNumber.Round(isotope.Value / divisor);
                if (amount.IsZero)
                {
                    combined.Remove(isotope.Key);
                }
            ...
            return new Formula((short)HugeNumber.Round(value.Charge / divisor), combined);
        }
        ...
        /// cref="short.MaxValue"/> or <see cref="short.MinValue"/>, or the amount of any nuclide
        /// exceeds <see cref="ushort.MaxValue"/>.</exception>
        public static Formula Multiply(Formula value, HugeNumber factor)
        {
            if (factor < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(factor));
            ...
            foreach (var isotope in combined.ToList())
            {
                var amount = HugeNumber.Round(isotope.Value * factor);
                if (amount.IsZero)
                {
                    combined.Remove(isotope.Key);
                }
            ...
            return new Formula((short)HugeNumber.Round(value.Charge * factor), combined);
        }
        ...
        /// cref="short.MaxValue"/> or <see cref="short.MinValue"/>, or the amount of any nuclide
        /// exceeds <see cref="ushort.MaxValue"/>.</exception>
        public Formula Divide(HugeNumber divisor) => Divide(this, divisor);
        ...
        {
            info.AddValue(nameof(Charge), Charge);
            info.AddValue(nameof(_isotopes), _isotopes);
        }
        ...
        /// cref="short.MaxValue"/> or <see cref="short.MinValue"/>, or the amount of any nuclide
        /// exceeds <see cref="ushort.MaxValue"/>.</exception>
        public Formula Multiply(HugeNumber factor) => Multiply(this, factor);
        ...
        public override string ToString()
        {
            ...
            if (Charge != 0)
            {
                sb.Append(Charge.ToSuperscript(positiveSign: true, postfixSign: true));
            }
            if (sb.Length == 0)
            {
                sb.Append("<empty>");
        ...
        /// cref="short.MaxValue"/> or <see cref="short.MinValue"/>, or the amount of any nuclide
        /// exceeds <see cref="ushort.MaxValue"/>.</exception>
        public static Formula operator *(Formula value, HugeNumber factor) => Multiply(value, factor);
        ...
        /// cref="short.MaxValue"/> or <see cref="short.MinValue"/>, or the amount of any nuclide
        /// exceeds <see cref="ushort.MaxValue"/>.</exception>
        public static Formula operator /(Formula value, HugeNumber divisor) => Divide(value, divisor);
