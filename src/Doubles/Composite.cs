    ...
    public class Composite : IMaterial, ISerializable, IEquatable<Composite>
    {
        ...
        public IReadOnlyDictionary<ISubstanceReference, decimal> Constituents
        {
            get
            {
                ...
                {
                    var ratio = Mass.IsNearlyZero() ? 0 : (decimal)(component.Mass / Mass);
        ...
        private Composite(SerializationInfo info, StreamingContext context) : this(
            ...
            (double?)info.GetValue(nameof(Mass), typeof(double?)),
        ...
        public IMaterial AddComponent(IMaterial material, decimal proportion = 0.5m, int index = -1)
        {
            ...
            var targetMass = (decimal)massTotal * proportion;
            var massFraction = targetMass / (decimal)material.Mass;
            if (index < 0)
            {
        ...
        public IMaterial CopyComponent(int index, decimal proportion = 0.5m)
        {
            ...
            var massTotal = (decimal)Components.Sum(x => x.Mass);
            var targetMass = massTotal * proportion;
            var massFraction = targetMass / (decimal)Components[index].Mass;
            var ratio = 1 - proportion;
            var list = new List<IMaterial>();
            for (var i = 0; i < Components.Count; i++)
            {
        ...
        public IMaterial GetClone(decimal massFraction = 1)
        {
            ...
            else
            {
                ...
                if (_mass.HasValue)
                {
                    if (massFraction != 1)
                    {
                        mass = _mass * (double)massFraction;
                    }
                    else
                    {
                        mass = _mass;
        ...
        public override int GetHashCode()
        {
            ...
            hashCode = (hashCode * -1521134295) + (_density?.GetHashCode() ?? 0);
            hashCode = (hashCode * -1521134295) + (_mass?.GetHashCode() ?? 0);
            hashCode = (hashCode * -1521134295) + (_temperature?.GetHashCode() ?? 0);
